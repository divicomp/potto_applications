...
def test_linear(num_samples=10):
    ...
    start = time.time()
    a = evaluate(integral, num_samples=num_samples)
    end = time.time()
    eval_time = end - start
    print(f'evaluation duration: {eval_time}')


def test_quadratic(num_samples=10):
    ...
    start = time.time()
    _,  integral = reverse_deriv(integral_xy, Tup(Const(1)), output_list=deriv_list)
    integral = reduce_to_base(integral)
    end = time.time()
    compile_time = end - start
    print(f'Compile duration: {compile_time}')
...
if __name__ == "__main__":
    ...
    test_linear(num_samples=100)
    print("quadratic color test")
    test_quadratic(num_samples=100)
